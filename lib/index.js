...
function diffPlugin(schema, options) {
   ...
   * @param {string} [options.optionKey=diff] - the path options key to mark paths for inclusion in monitoring for modification. If no paths are provided or tagged, document modification is monitored.
   * @param {array} [options.optionKey] - the paths paths for monitoring for modification. If no paths are provided or tagged, document modification is monitored.
  ...
  const paths = options.paths || Object.keys(schema.paths).filter(path => {
  ...
  const getDiffFn = _.partial(getDiff, options.snapShotPath, paths);
