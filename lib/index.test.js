...
describe('Mongoose plugin: diff', () => {
  ...
  it('should add a `getDiff` method', () => {
    ...
    const User = connection.model('User', schema, 'Users');
    const user = new User();
  ...
  describe('with defaults', () => {
    ...
    it('should maintain distinct differences for each document', () => {
      const newUser1 = new User();
      const newUser2 = new User();

      newUser1.set('username', faker.internet.userName());

      // NOTE: Due to a Jest bug the following test would be a false fail
      // https://github.com/facebook/jest/issues/711
      // expect(newUser1.getDiff()).not.toEqual(newUser2.getDiff());
      expect(newUser1.getDiff()).toHaveProperty('username');
      expect(newUser2.getDiff()).not.toHaveProperty('username');
    });
